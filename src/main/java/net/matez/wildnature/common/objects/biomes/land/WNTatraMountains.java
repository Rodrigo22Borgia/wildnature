...
public class WNTatraMountains extends WNBiome { 
    public WNTatraMountains() { 
        super(WNBiomes.TatraMountains, Biome.Precipitation.RAIN, Biome.BiomeCategory.EXTREME_HILLS, 0.22F, 0.975F, NORMAL_MUSIC, 0x9ED96F /*0xC7D975*/, 0x5DB259 /*0x7FBC70*/);
    }
    ...
    public void applyCustomFeatures(BiomeLoadingEvent event) {
        // -- TREES
        addTree(event, WNExtraTrees.MINI_BALD_PINE, 2, 0.5F, 2);
        addTree(event, WNExtraTrees.MINI_BALD_PINE, 5, 0.5F, 2);
        addTree(event, WNExtraTrees.MINI_PINE, 2, 0.5F, 2);
        addTree(event, WNExtraTrees.TREE_TAIGA, 1, 0.5F, 2);

        // -- FLOWERS
        //addPlant(event, FlowerType.BROWN_PODZOL, 0);
        addPlant(event, FlowerType.CROCUS_PURPLE, 12);
        addPlant(event, FlowerType.CROCUS_WHITE, 3);
        addPlant(event, FlowerType.FIREWEED, 2);
        //addPlant(event, FlowerType.OVERGROWN_STONE, 0);
}


    @Override
    public SurfaceRules.RuleSource getSurface() {
        return WNSurface.STONE_STONE_GRAVEL_CONFIG;
    }
}
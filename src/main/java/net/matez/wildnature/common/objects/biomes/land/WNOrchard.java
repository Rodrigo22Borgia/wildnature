...
public class WNOrchard extends WNBiome { 
    public WNOrchard() { 
        super(WNBiomes.Orchard, Biome.Precipitation.RAIN, Biome.BiomeCategory.FOREST, 0.6F, 0.8F, NORMAL_MUSIC, 0, 0);
    }
    ...
    public void applyCustomFeatures(BiomeLoadingEvent event) {
        ...
        addTree(event, WNExtraTrees.TREE_OAK, 4, 0.5F, 2);

        // -- FLOWERS
        addPlant(event, FlowerType.GERANIUM_PINK, 6);
        addPlant(event, FlowerType.GERANIUM_RED, 6);
        addPlant(event, FlowerType.GERANIUM_WHITE, 4);
        //addPlant(event, FlowerType.GRAPE_VINE_YELLOW, 0);
        addPlant(event, FlowerType.HOLLYHOCK_RED, 4);
        addPlant(event, FlowerType.MARIGOLD_ORANGE, 3);
}

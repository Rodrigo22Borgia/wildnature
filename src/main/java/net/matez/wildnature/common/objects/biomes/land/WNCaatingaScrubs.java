...
public class WNCaatingaScrubs extends WNBiome { 
    public WNCaatingaScrubs() { 
        super(WNBiomes.CaatingaScrubs, Biome.Precipitation.NONE, Biome.BiomeCategory.SAVANNA, 1F, 0.0F, NORMAL_MUSIC, 0xD0D67B /*0xD6C495*/, 0);
    }
    ...
    public MobSpawnSettings.Builder buildMobSpawn() {
        ...
        spawnBuilder.addSpawn(MobCategory.CREATURE, new MobSpawnSettings.SpawnerData(EntityType.DONKEY, 1, 1, 1));
    ...
    public void applyCustomFeatures(BiomeLoadingEvent event) {
        // -- TREES
        addTree(event, WNExtraTrees.ACACIA, 7, 0.5F, 2);
        addTree(event, WNExtraTrees.BAOBAB, 1, 0.5F, 2);
        //addTree(event, WNExtraTrees.DEAD, 12, 0.5F, 2);

        // -- FLOWERS
        addPlant(event, FlowerType.CORDYLINE_AUSTRALIS, 4);
        addPlant(event, FlowerType.FESTUCA, 4);
        addPlant(event, FlowerType.RED_SAND_VERBENA, 12);
        addPlant(event, FlowerType.SUCCULENT, 2);
}


    @Override
    public SurfaceRules.RuleSource getSurface() {
        return WNSurface.SAND_CONFIG;
    }
}